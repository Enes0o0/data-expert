...
# API anahtarlarını constants.py dosyasından al
...
marketing_agent = Agent(
    ...
    memory=chat_memory  # memory agent
)
...
def scrape_toms_hardware():
    ...
    try:
        response = requests.get(url, verify=True)  # SSL doğrulamasını etkinleştir
    ...
    for row in rows:
        ...
        if len(columns) >= 7:  # En az 7 sütun içermesi gerekiyor
            data.append([
...
if prompt:
    ...
    task = Task(description=improved_prompt, agent=marketing_agent, expected_output="Veri analiz sonuçları")
    ...
    if st.button("Oluştur"):
        with st.spinner("Yanıt oluşturuluyor..."):
            ...
            # Task tamamlandığında çıktıyı al
            ...
            # İyi formatlanmış çıktı
            st.markdown("### Tom's Hardware GPU Veri Analiz Sonuçları:")
            st.write(result)

            # Tom's Hardware verilerini Excel'e yazma
